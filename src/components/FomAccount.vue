...
<template>
   <form action="">
    <div class="mb-4">
        ...
        <input min="0" id="total" type="number" placeholder="S/." class="mt-1 p-1 border border-gray-300 rounded-md"
            v-model="total"
        >
    </div>
    <div class="mb-4">
        <label for="propina" class="block text-sm font-medium text-gray-700">Porcenataje de Propina</label>
        <input min="0"  id="propina" type="number" placeholder="(%)" class="mt-1 p-1 border border-gray-300 rounded-md"
            v-model="tip"
        >
    </div>
    <div class="mb-1">
        <label for="numPersonas" class="block text-sm font-medium text-gray-700">Número de Personas</label>
        <input min="0" id="numPersonas" type="number" placeholder="n°" class="mt-1 p-1 border border-gray-300 rounded-md"
            v-model="numPeople"
        >
    </div>
    ...
    <button  v-if="store.people.length>0" class=" m-5 p-1 bg-violet-600 w-40 rounded-lg text-gray-100" type="button"
        @click="clean"
        >Limpiar todo
    </button>
</form>
